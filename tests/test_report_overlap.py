...
from src.gowri_proj.parser import ReportMeta, TIDY_COLUMNS


def _meta(start, end):
    return ReportMeta(company="TEST PHARMACY", location="TEST CITY", period_start=date.fromisoformat(start), period_end=date.fromisoformat(end))


def _df(*skus):
    rows = [
        {"brand": "BRAND", "sku": sku, "opening_stock": 1.0, "purchase": 0.0, "purchase_free": 0.0,
         "other_receipt": 0.0, "sales": 1.0, "sales_free": 0.0, "other_issue": 0.0, "closing_stock": 1.0, "value": 10.0}
        for sku in skus
    ]
...
def test_a_wider_upload_supersedes_a_narrower_overlapping_report(tmp_path):
    with db.connect(str(tmp_path / "test.db")) as conn:
        ...
        result = db.import_report(conn, _df("A", "B"), _meta("2026-06-01", "2026-06-14"), "week1-2.xlsx")
