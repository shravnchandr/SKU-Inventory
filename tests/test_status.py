...
def test_low_stock_below_threshold():
    assert _status(closing_stock=5, is_dead=False, days_of_cover=5, low_stock_days=15) == "low_stock"


def test_overstock_above_threshold():
    assert _status(closing_stock=5, is_dead=False, days_of_cover=200, overstock_days=90) == "overstock"


def test_healthy_in_the_middle():
    assert _status(closing_stock=5, is_dead=False, days_of_cover=40, low_stock_days=15, overstock_days=90) == "healthy"
...
def test_boundaries_are_exclusive_not_inclusive():
    ...
    assert _status(closing_stock=5, is_dead=False, days_of_cover=15, low_stock_days=15, overstock_days=90) == "healthy"
    assert _status(closing_stock=5, is_dead=False, days_of_cover=90, low_stock_days=15, overstock_days=90) == "healthy"


def test_custom_thresholds_are_respected():
    assert _status(closing_stock=5, is_dead=False, days_of_cover=3, low_stock_days=2, overstock_days=10) == "healthy"
    assert _status(closing_stock=5, is_dead=False, days_of_cover=1, low_stock_days=2, overstock_days=10) == "low_stock"
